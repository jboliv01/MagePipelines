...
def load_parquet_from_url(download_info, *args, **kwargs) -> pd.DataFrame:
    ...
    def safe_convert_to_timestamp(column, max_year=9999):
        ...
        # Create a boolean mask for dates with year less than max_year
        mask = dt_column.dt.year < max_year

        # Apply condition using vectorized operations
        # Assuming you want to truncate the dates with year >= max_year
        # Convert to UNIX timestamp (number of seconds since 1970-01-01)
        dt_column[~mask] = dt_column[~mask].astype('int64') // 10**9
        return dt_column
